...
public class AddController implements Initializable {
    ...
    @FXML
    private void tableDeliveryComponentClick(MouseEvent mouseEvent) {
        if ( mouseEvent.getClickCount() == 2 && mouseEvent.getButton().equals(MouseButton.PRIMARY) ){
    ...
    private void insertStoreComponent(int idDeliveryArrival , double payingDelivery){
        ...
        for (int i = 0; i < dataTable.size(); i++) {
            ...
            insertComponentStoreRawMaterial(componentStore);
        }
    }
    ...
    private boolean insertComponentStoreRawMaterial(ComponentStore componentStore){
        boolean insert = false;
        try {
            insert = componentStoreRawMaterialOperation.insert(componentStore);
            return insert;
        }catch (Exception e){
            e.printStackTrace();
            return insert;
