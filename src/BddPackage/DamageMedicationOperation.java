...
public class DamageMedicationOperation extends BDD<Damage> {
    ...
    public int insertId(Damage damage) {
        connectDatabase();
        int ins = 0;
        String query = "INSERT INTO اتلاف_الادوية (معرف_الدواء, تاريخ_التلف, الكمية, السبب) VALUES (?,?,?,?);";
        try {
    ...
    public boolean update(Damage o1, Damage o2) {
        connectDatabase();
        boolean upd = false;
        String query = "UPDATE اتلاف_الادوية SET تاريخ_التلف = ?, الكمية = ?, السبب = ? WHERE المعرف = ?;";
        try {
            PreparedStatement preparedStmt = conn.prepareStatement(query);
    ...
    public boolean delete(Damage o) {
        connectDatabase();
        boolean del = false;
        String query = "DELETE FROM اتلاف_الادوية WHERE المعرف = ?;";
        try {
            PreparedStatement preparedStmt = conn.prepareStatement(query);
    ...
    public ArrayList<Damage> getAll() {
        ...
        String query = "SELECT * FROM اتلاف_الادوية ";
        try {
            PreparedStatement preparedStmt = conn.prepareStatement(query);
            ...
            while (resultSet.next()){
                ...
                damage.setIdProduct(resultSet.getInt("معرف_الدواء"));
